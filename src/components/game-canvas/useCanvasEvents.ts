      ...
      for (const planet of gameState.planets) {
        ...
        if (distance <= planet.radius) {
          ...
          if (selectedPlanets.length > 0) {
            // Send first fleet immediately for responsive feeling
            if (selectedPlanets.length > 0) {
              sendFleet(planet.id, fleetPercentage, selectedPlanets[0]);
            }
            
            // Send remaining fleets with minimal delays
            if (selectedPlanets.length > 1) {
              for (let i = 1; i < selectedPlanets.length; i++) {
                const delay = i * 50; // Shorter delay (50ms) for responsive feel
                setTimeout(() => {
                  sendFleet(planet.id, fleetPercentage, selectedPlanets[i]);
                }, delay);
              }
            }
          }
    ...
    if (isSelecting && selectionBox) {
      ...
      if (maxX - minX > 5 || maxY - minY > 5) {
        ...
        if (e.shiftKey) {
          ...
          setSelectedPlanets((prevSelected: string[]) => {
